...
streamPipe ::
  forall eff stream dest chars. (ReadStreamChar stream chars, WriteStreamChar dest chars) =>
    stream eff -> dest eff -> Eff (streaming :: Streaming | eff) Unit

streamPipe = runMethodEff1 "pipe"

streamRead ::
  forall eff stream chars. (ReadStreamChar stream chars) =>
    stream eff -> N ReadSize -> Eff (streaming :: Streaming | eff) (N chars)

streamRead = runMethodEff1 "read"

streamPause ::
  forall eff stream chars. (ReadStreamChar stream chars) =>
    stream eff -> Eff (streaming :: Streaming | eff) Unit
...
streamWrite ::
  forall eff stream chars. (WriteStreamChar stream chars) =>
    stream eff -> chars -> Eff (streaming :: Streaming | eff) Boolean
...
streamRead ::
  forall eff stream val. (ReadStream stream val) =>
    stream eff -> Eff (streaming :: Streaming | eff) (N val)
...
streamWrite ::
  forall eff stream val. (WriteStream stream val) =>
    stream eff -> val -> Eff (streaming :: Streaming | eff) Boolean
